...
function animate() {
  setTimeout(function () {
    if (!dontanimate) requestAnimationFrame(animate);
  }, 1000 / 30);
...
function onDocumentMouseDown(e) {
  ...
  if (e.button === 2) board.rightclick();
  else {
...
function output(e) {
  if (typeof DEBUG !== "undefined" && DEBUG) console.log("output:" + e);
}
...
function load() {
  ...
  if (!board.scratch && !board.observing) {
    alert(
      "warning",
      "TPS/PTN won't be displayed in the middle of an online game"
    );
    return;
  }
...
function downloadNotation() {
  $("#download_notation").attr(
    "href",
    "data:text/plain;charset=utf-8," + encodeURIComponent(getNotation())
  );
}
