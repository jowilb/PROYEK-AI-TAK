...
var materials = {
  ...
  getWhiteSquareTextureName: function () {
    return (
      this.board_texture_path + "white_" + this.white_sqr_style_name + ".png"
    );
  },
  getBlackSquareTextureName: function (styleName) {
    return (
      this.board_texture_path + "black_" + this.black_sqr_style_name + ".png"
    );
  },
  getWhitePieceTextureName: function () {
    return (
      this.pieces_texture_path +
      "white_" +
      this.white_piece_style_name +
      "_pieces.png"
    );
  },
    ...
    if (materials.piecesLoaded === 4) {
      ...
      for (i = 0; i < board.piece_objects.length; i++) {
        if (board.piece_objects[i].iscapstone) {
          ...
          board.piece_objects[i].material = board.piece_objects[i].iswhitepiece
            ? materials.white_piece
            : materials.black_piece;
        }
      }
    }
  },
...
var boardFactory = {
  makeSquare: function (file, rankInverse, scene) {
    ...
    var square = new THREE.Mesh(
      geometry,
      (file + rankInverse) % 2 ? materials.white_sqr : materials.black_sqr
    );
    square.position.set(
...
var board = {
  ...
  calculateConnectedStone: function (pieceColor) {
    ...
    const checkVisited = (file, rank, visited) => {
      for (let i = 0; i < visited.length; i++) {
        if(visited[i][0] == file && visited[i][1] == rank) return true
      }
      return false
    }

    const checkpointUpdate = (file, rank, type) => {
      for (let i = 0; i < checkpoint.length; i++) {
        if(checkpoint[i][0] == file && checkpoint[i][1] == rank){
          if(checkpoint[i][2] == type) return true
        }
      }
      return false
    }
    ...
    for (let i = 0; i < this.size; i++) {
      for (let j = 0; j < this.size; j++) {
        if(this.sq[i][j].length === 0) continue;
        let cur_st = this.sq[i][j];
        let ctop = cur_st[cur_st.length - 1];
        if(ctop.iswhitepiece && !ctop.isstanding && pieceColor === "white"){
          if(i == 0 || checkpointUpdate(i,j,true)) startFile = true
          else startFile = false
          if(j == 0 || checkpointUpdate(i,j,false)) startRank = true
          else startRank = false
          let visited = []
          for (let k = 0; k < directions.length; k++) {
            let file = i + directions[k][0];
            let rank = j + directions[k][1];
            let score
            if(ctop.iscapstone) score = 5
            else score = 1
            while(file >= 0 && file < this.size && rank >= 0 && rank < this.size){
              if(this.sq[file][rank].length !== 0){
                let ctop2 = this.sq[file][rank][this.sq[file][rank].length - 1];
                if(ctop2.iswhitepiece && !ctop2.isstanding && !checkVisited(file,rank,visited)){
                  if(ctop2.iscapstone) score *=3
                  else score*=2;
                  visited.push([file,rank])
                }
                else if(startFile || startRank){
                  if(startFile) checkpoint.push([file,rank,true])
                  if(startRank) checkpoint.push([file,rank,false])
                }
                else break
              }
              else break;
            ...
            if(startFile && file == this.size - 1){
              score *=5
            }
            if(startRank && rank == this.size - 1){
              score *=5
            }
            if(score > maxScore) maxScore = score
          }
        }
        else if(!ctop.iswhitepiece && !ctop.isstanding && pieceColor === "black"){
          if(i == 0 || checkpointUpdate(i,j,true)) startFile = true
          else startFile = false
          if(j == 0 || checkpointUpdate(i,j,false)) startRank = true
          else startRank = false
    ...
    for (let i = 0; i < this.size; i++) {
      for (let j = 0; j < this.size; j++) {
        if(this.sq[j][i].length === 0) continue;
        let cur_st = this.sq[j][i];
        let ctop = cur_st[cur_st.length - 1];
        if(ctop.iswhitepiece && !ctop.isstanding && pieceColor === "white"){
          if(i == 0 || checkpointUpdate(i,j,true)) startFile = true
          else startFile = false
  ...
  calculateStaticBoardEvaluator: function () {
    ...
    var whiteStandstone = this.calculateStandstone("white") * 7;
    var whiteCapstone = this.calculateCapstone("white") * 10;
    var whitepieceConnected = this.calculateConnectedStone("white")*15;
    var whitepieceControlled = this.calculateControlledStone("white")*9;
    var whitepieceCenter = this.calculatedCenterStone("white") * 3;
    ...
    var blackStandstone = this.calculateStandstone("black") * 7;
    var blackCapstone = this.calculateCapstone("black") * 10;
    var blackpieceConnected = this.calculateConnectedStone("black")*15;
    var blackpieceControlled = this.calculateControlledStone("black")*9;
    var blakcpieceCenter = this.calculatedCenterStone("black") * 3;
    ...
    if (isMaximizingPlayer) {
      ...
      if (allPiece != -1) {
        for (let i = 0; i < allPiece.length; i++) {
          ...
          let moves2 = this.getSingleAllPosibleMoves(allPiece[i].onsquare);
          let piece = this.getPositionPiece(
            allPiece[i].onsquare.file,
            allPiece[i].onsquare.rank
          ); //get index piece
          for (let j = 0; j < moves2.length; j++) {
            this.sq[moves2[j].file][moves2[j].rank].push(
              this.piece_objects[piece]
            );
            let eval = this.calculateMinimax(depth - 1, false);
            this.sq[moves2[j].file][moves2[j].rank].pop();
            maxEval4 = Math.max(maxEval4, eval);
          }
        }
      }

      return Math.max(maxEval, maxEval2, maxEval3,maxEval4);
    } else {
      //Min
      let minEval = Infinity;
      let minEval2 = Infinity;
      let minEval3 = Infinity;
      let minEval4 = Infinity;
      let moves = this.getAllEmptyPositions();
      ...
      if (allPiece != -1) {
        for (let i = 0; i < allPiece.length; i++) {
          ...
          for (let j = 0; j < moves2.length; j++) {
            ...
            let eval = this.calculateMinimax(depth - 1, true);
            this.sq[moves2[j].file][moves2[j].rank].pop();
            minEval4 = Math.min(minEval4, eval);
          }
        }
      }
      return Math.min(minEval, minEval2, minEval3,minEval4);
    }
  },

  getBestMovement: function (depth, isMaximizingPlayer) {
  ...
  choosePiece: function () {
    ...
    var randomIndex = chooseBestPiece; //mengambil hasil dari perhitungan minimax
  ...
  choosePosition: function () {
    ...
    if (this.movecount == 0) {
      ...
      if (randomNumbers == 0) {
        position = 0; // Position A5
      } else if (randomNumbers == 1) {
        position = 4; // Position A1
      } else if (randomNumbers == 2) {
        position = 20; // Position E5
              ...
              if (countblack >= 3) {
                for (let k = 0; k < this.size; k++) {
                  if (this.sq[k][i].length === 0) {
                    position = this.getPositionBoard(k, i);
                    block = true;
                    break;
                  }
                }
        ...
        for (let i = 0; i < this.size; i++) {
          for (let j = 0; j < this.size; j++) {
            ...
            if (ctop.iswhitepiece) {
              ...
              if (valid == "up") {
                position = this.getPositionBoard(i, j + 1);
                choose = 1;
                break;
              }
              if (valid == "down") {
              ...
              if (valid == "left") {
                position = this.getPositionBoard(i - 1, j);
                choose = 1;
                break;
              }
              if (valid == "right") {
  ...
  getPositionBoard: function (file, rank) {
    let matchingBoardObjects = this.board_objects.filter(
      (obj) => obj.file === file && obj.rank === rank
    );
    let index = this.board_objects.indexOf(matchingBoardObjects[0]);
    return index;
  },
  ...
  incmovecnt: function () {
    ...
    this.ismymove = this.checkifmymove();
    $("#undo")
      .removeClass("i-requested-undo")
      .removeClass("opp-requested-undo")
      .addClass("request-undo");
  },
  ...
  leftclick: function () {
    ...
    if (intersects.length > 0) {
      ...
      if (!obj.isboard && !obj.onsquare && !this.isPlayEnded) {
        ...
        if (obj.iswhitepiece !== this.is_white_piece_to_move()) return;
        //no capstone move on 1st moves
        if (Math.floor(this.movecount / 2) === 0 && obj.iscapstone) return;
      ...
      //select stack ... no stack selection on 1st moves
        ...
        !this.selectedStack &&
        Math.floor(this.movecount / 2) !== 0 &&
        !this.isPlayEnded
      ) {
        var sq = obj;
        if (!obj.isboard) {
          if (!obj.onsquare) return;
          sq = obj.onsquare;
        }
  ...
  mousemove: function () {
    ...
    raycaster.setFromCamera(mouse, camera);
    if (!this.selected && !this.selectedStack) return;

    var intersects = raycaster.intersectObjects(scene.children);

    if (intersects.length > 0) {
      var obj = intersects[0].object;
      if (!obj.isboard && !obj.onsquare) return;
      if (!obj.isboard) obj = obj.onsquare;

      if (this.selectedStack) {
        var tp = this.top_of_stack(obj);
        if (tp && tp.iscapstone) return;
        if (
          tp &&
          tp.isstanding &&
        ...
        if (this.move.dir === "U" && rel !== "OUTSIDE") this.highlight_sq(obj);
        else if (this.move.dir === rel || rel === "O") this.highlight_sq(obj);
  ...
  sendmove: function (e) {
    if (!this.server || this.scratch) return;
  ...
  serverPmove: function (file, rank, caporwall) {
    ...
    dontanimate = true;
    fastforward();
    var obj = this.getfromstack(
      caporwall === "C",
      this.is_white_piece_to_move()
    );
  ...
  notate: function (txt) {
    ...
    if (txt === "load") {
      ...
      if (this.movecount % 2 === 1) {
        ...
        cell1.innerHTML =
          '<a href="#" onclick="board.showmove(' +
          this.movecount +
          ');"><span class="curmove moveno' +
          (this.movecount - 1) +
          '">' +
          txt +
          "</span></a>";
      } else {
        var row = this.insertNewNotationRow(
          Math.floor(this.movecount / 2 + 1) - 1
        );
        var cell1 = row.cells[1];
  ...
  notateMmove: function (stf, str, endf, endr, nos) {
    ...
    var move = tot + this.squarename(stf, str).toLowerCase() + dir + "" + lst;
    this.notate(move);
  },
  ...
  pushPieceOntoSquare: function (sq, pc) {
    ...
    if (pc.isstanding) {
      if (pc.iscapstone)
        pc.position.y =
          sq_height / 2 + capstone_height / 2 + piece_height * st.length;
      else
        pc.position.y =
          sq_height / 2 + piece_size / 2 + piece_height * st.length;
  ...
  clear: function () {
    ...
    $("#draw")
      .removeClass("i-offered-draw")
      .removeClass("opp-offered-draw")
      .addClass("offer-draw");
    stopTime();
  ...
  loadptn: function (ptn) {
    ...
    for (var ply = 0; ply < parsed.moves.length; ply++) {
      ...
      if ((match = /^([SFC]?)([a-h])([0-8])$/.exec(move)) !== null) {
        ...
        (match = /^([1-9]?)([a-h])([0-8])([><+-])(\d*)$/.exec(move)) !== null
      ) {
        var count = match[1];
        var file = match[2].charCodeAt(0) - "a".charCodeAt(0);
        var rank = parseInt(match[3]) - 1;
  ...
  loadtps: function (tps) {
    ...
    var tpsRE =
      /\[(TPS\s*)?\"?\s*([,x12345678SC\/]+)(\s+([\d+]))?(\s+(\d+|-))?\s*\"?\s*\]/;
    ...
    if (!result) {
      ...
      if (!playerToMove) {
        ...
        alert(
          "warning",
          "Invalid TPS - player turn must be 1 or 2 - not " + playerTurn
        );
        return;
      }
    ...
    var assumedMoveCount = this.moveCountCalc(
      moveNumber,
      moveNumber,
      playerToMove
    );
    ...
    if (p1Cnt == 0 && p2Cnt == 0) {
      ...
      if (assumedMoveCount < minMoves) {
        ...
        infoMsg =
          "Initializing move number to correpond with the number of pieces on the board.";
      }
    ...
    document.getElementById("player-opp").className =
      this.mycolor === "white" ? "selectplayer" : "";
    document.getElementById("player-me").className =
      this.mycolor === "black" ? "selectplayer" : "";
  ...
  count_pieces_on_board: function (player) {
    ...
    for (i = 0; i < pos.length; i++) {
      ...
      for (s = 0; s < pieces.length; s++) {
        if (
          (player === WHITE_PLAYER && pieces[s] === pieces[s].toUpperCase()) ||
    ...
    for (var i = 0; i < rowCnt; i++, currRow--) {
      ...
      for (var j = 0; j < cellDescriptors.length; j++) {
        if (currCol > rowCnt) {
          alert(
            "warning",
            "Invalid TPS - too many cells in row " +
              currRow +
              '... "' +
              rowDescriptors[i] +
              '"'
        ...
        if (!cellResult) {
          alert(
            "warning",
            "Invalid TPS - cell descriptor in row " +
              currRow +
              '... "' +
              cellDescriptor +
              '" in "' +
        ...
        if (cellResult[2]) {
          ...
          if (currCol > rowCnt + 1) {
            alert(
              "warning",
              "Invalid TPS - too many empty cells at end of row " +
                currRow +
                '... "' +
                rowDescriptors[i] +
                '"'
        ...
        for (var k = 0; k < stackDescriptor.length; k++) {
          ...
          if (pc === null || typeof pc === "undefined") {
            alert(
              "danger",
              "Invalid TPS - too many pieces for player " +
                playerNum +
                " at row " +
                currRow +
                ', "' +
